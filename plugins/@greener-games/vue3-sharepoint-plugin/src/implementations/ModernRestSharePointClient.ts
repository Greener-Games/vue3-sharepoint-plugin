    ...
    postquery?: {
        PrimaryQueryResult: {
            RelevantResults: {
                Table: {
                    Rows: {
                        results: Array<{ Cells: Array<{ Key: string, Value: unknown }> }>
                    }
                }
                TotalRows: number
            }
...
export class ModernRestSharePointClient implements ISharePointClient {
  ...
  async search<T>(opts: SearchRequestOptions, signal?: AbortSignal): Promise<SearchResult<T>> {
    ...
    const response = await this.request<{ PrimaryQueryResult?: { RefinementResults?: { Refiners?: Array<{ Name: string, Entries: Array<{ RefinementName: string, RefinementCount: number, RefinementToken: string }> }> }, RelevantResults?: { RowCount: number, TotalRows: number, Table?: { Rows?: Array<{ Cells: Array<{ Key: string, Value: string }> }> } } } }>(`/_api/search/postquery`, {
      method: 'POST',
      body: payload,
      isWrite: true,
      skipMetadata: true,
    ...
    const postquery = response
    if (!postquery || !postquery.PrimaryQueryResult) {
    ...
    if (refinerData && refinerData.Refiners) {
      refiners = refinerData.Refiners.map((r) => ({
        name: r.Name, // e.g., "RefinableString00"
        values: r.Entries.map((e) => ({
    ...
    const rows = postquery.PrimaryQueryResult.RelevantResults?.Table?.Rows || []

    let items: Record<string, unknown>[] = rows.map((r) => {
      const map: Record<string, unknown> = {}
      r.Cells.forEach((c) => (map[c.Key] = c.Value))
    ...
    return {
      ...
      totalHits: postquery.PrimaryQueryResult.RelevantResults?.TotalRows || 0,
      startRow: opts.startRow || 0,
      refiners: refiners,
    }
  }
  ...
  async searchTerm(
    ...
    try {
      ...
      const response = await this.request<Array<{ id: string, name: string, names?: Array<{ name: string }> }>>(endpoint, { signal })
    ...
    if (options?.getAll) {
      ...
      while (nextUrl) {
        ...
        if (response && response.d) {
          ...
          nextUrl = undefined /* handled below */
  ...
  async updateFileMetadata(url: string, payload: Record<string, unknown>, signal?: AbortSignal): Promise<void> {
    ...
    const response = await this.request<Array<{ FieldName: string, ErrorCode: number, ErrorMessage: string }>>(
  ...
  private async getRequestDigest(customBaseUrl?: string): Promise<string> {
    ...
    const h = new Headers({ Accept: 'application/json;odata=nometadata' })
    if (this.authProvider) {
        const authHeaders = await this.authProvider()
    ...
    if (!res.ok) return ''
    const response = (await res.json()) as { FormDigestValue: string, FormDigestTimeoutSeconds: number }

    const token = response.FormDigestValue
    if (isDefault) {
        this.digestCache = token
        this.digestExpiry =
          Date.now() +
          response.FormDigestTimeoutSeconds * 1000 -
          60000
    }
    return token
  }
  ...
  async searchUsers(query: string, signal?: AbortSignal): Promise<UserInfo[]> {
    ...
    try {
      const response = await this.request<string>(pickerEndpoint, {
        method: 'POST',
        body: payload,
        isWrite: true,
        signal
      ...
      const pickerResults = JSON.parse(response) as Array<{ DisplayText: string, EntityData: { Email?: string }, Description: string, Key: string }>;

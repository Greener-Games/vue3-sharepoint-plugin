...
export class MockSharePointClient implements ISharePointClient {
  ...
  async search<T>(
    ...
    if (qRaw && qRaw !== '*') {
      ...
      allItems = allItems.filter((item: Record<string, unknown>) => {
        const titleMatch = (typeof item.Title === 'string' ? item.Title : '')
          .toLowerCase()
          .includes(q);
        ...
        const summaryMatch = String((item.HitHighlightedSummary as string) || '')
          .toLowerCase()
          .includes(q);
    ...
    if (opts.filters) {
      Object.entries(opts.filters).forEach(([key, value]) => {
        ...
        allItems = allItems.filter((item) => {
          ...
          const normalizedItemVal =
            typeof itemVal === 'object' && itemVal !== null
              ? JSON.stringify(itemVal).toLowerCase()
              : String(itemVal).toLowerCase();

          if (Array.isArray(value)) {
            return value.some((v) => normalizedItemVal.includes(String(v).toLowerCase()));
          }
  ...
  async searchUsers(query: string, _abortSignal?: AbortSignal): Promise<UserInfo[]> {
    ...
    return allUsers.filter(
      (u) => u.Title.toLowerCase().includes(q) || u.Email.toLowerCase().includes(q),
    );
  }
  ...
  async ensureUser(loginName: string, _abortSignal?: AbortSignal): Promise<UserInfo> {
    ...
    const user = siteUsers.find((u) => u.LoginName === loginName || u.Email === loginName);
    if (user) return user;
  ...
  async sendEmail(props: EmailProperties): Promise<void> {
    await this.wait();
    this.logger.log(`📧 Sent Email to [${props.To.join(', ')}]: ${props.Subject}`);
  }
  ...
  getVersionHistoryLinkByItem(listId: string, itemId: number, _webUrl?: string): string {
  ...
  async uploadFile(
    ...
    this.data.files[fullPath] = file instanceof ArrayBuffer ? new Blob([file]) : file;
  ...
  async executeBatch(builder: (batch: IBatch) => void, _abortSignal?: AbortSignal): Promise<void> {
    builder({
      createListItem: (list, payload) => {
        void this.createListItem(list, payload);
      },
      updateListItem: (list, id, payload) => {
        void this.updateListItem(list, id, payload);
      },
      deleteListItem: (list, id) => {
        void this.deleteListItem(list, id);
      },
      deleteFile: (url) => {
        void this.deleteFile(url);
      },
    });

...
export class RestSharePointClient implements ISharePointClient {
  ...
  async getWebInfo(signal?: AbortSignal): Promise<WebInfo> {
    const w = await this.request<WebInfo>(`/_api/web?$select=Id,Title,Url,Description`, {
      signal: signal,
    });
    return {
      Id: w.Id,
  ...
  private async processInternalBatch(requests: InternalBatchItem[], signal?: AbortSignal) {
    ...
    for (const req of requests) {
      ...
      if (req.headers) Object.entries(req.headers).forEach(([k, v]) => (body += `${k}: ${v}\r\n`));
      if (req.body) {
  ...
  async search<T>(opts: SearchRequestOptions, signal?: AbortSignal): Promise<SearchResult<T>> {
    ...
    const payload = {
      request: {
        ...
        Refiners: opts.refiners && opts.refiners.length > 0 ? opts.refiners.join(',') : undefined,
        HitHighlightedProperties: { results: ['Contents', 'Title'] },
        SummaryLength: 250,
        EnableStemming: true,
        TrimDuplicates: false,
        SortList:
          opts.sortList && opts.sortList.length > 0
            ? {
    ...
    const response = await this.request<SPVerboseResponse<Record<string, unknown>>>(
    ...
    let items: Record<string, unknown>[] = rows.map((r) => {
      ...
      let directUrl = [map.DefaultEncodingURL, map.PictureURL, map.OriginalPath, map.Path].find(
        (url) => typeof url === 'string' && !url.toLowerCase().includes('dispform.aspx'),
      ) as string | undefined;

      // If we have a filename in 'Title' and a 'Path', we can often swap out the Form part
      if (
        (!directUrl || directUrl.toLowerCase().includes('dispform.aspx')) &&
      ...
      if (opts.includeRelativePath && typeof map.DirectLink === 'string') {
        try {
          map.relativePath = decodeURIComponent(new URL(map.DirectLink).pathname);
        } catch {
      ...
      await Promise.all(
        items.map(async (item) => {
          if (
            ...
            (typeof item.ListItemId === 'string' || typeof item.ListItemId === 'number')
          ) {
            try {
              const params: string[] = [];
  ...
  async searchTerm(
    ...
    try {
      ...
      const response = await this.request<
        SPODataResponse<Array<{ id: string; name: string; names?: Array<{ name: string }> }>>
      >(endpoint, { signal });
  ...
  private buildKql(opts: SearchRequestOptions): string {
    ...
    if (txt !== '*') {
      ...
      const fields =
        opts.searchFields && opts.searchFields.length > 0
          ? opts.searchFields
          : ['Title', 'Filename'];
    ...
    if (opts.filters) {
      for (const [key, value] of Object.entries(opts.filters)) {
        ...
        if (opts.mapping) {
          const mapping = opts.mapping;
          const found = Object.keys(mapping).find((k) => mapping[k] === key);
          if (found) mp = found;
        }
  ...
  async getListItemById<T>(
    ...
    return await this.request<T>(`/_api/web/lists/getbytitle('${list}')/items(${id})${q}`, {
      signal,
    });
  }
  ...
  async downloadFile(url: string, signal?: AbortSignal): Promise<Blob> {
    ...
    const res = await fetch(`${apiRoot}/_api/web/getfilebyserverrelativeurl('${fullUrl}')/$value`, {
      headers,
      signal: signal,
    });
  ...
  async updateFileMetadata(
    ...
    const response = await this.request<
      SPVerboseResponse<{
        ValidateUpdateListItem: {
          results: Array<{ FieldName: string; ErrorCode: number; ErrorMessage: string }>;
        };
      }>
  ...
  async getList(listTitle: string, signal?: AbortSignal): Promise<ListInfo> {
    const l = await this.request<ListInfo>(`/_api/web/lists/getbytitle('${listTitle}')`, {
      signal,
    });
    return {
      Id: l.Id,
  ...
  private async getListEntityType(listTitle: string): Promise<string> {
    ...
    try {
      const list = await this.request<{ ListItemEntityTypeFullName: string }>(
  ...
  private async getRequestDigest(customBaseUrl?: string): Promise<string> {
    ...
    if (isDefault && this.digestCache && Date.now() < this.digestExpiry) return this.digestCache;
    ...
    if (!res.ok) return '';
    const response = (await res.json()) as SPVerboseResponse<{
      GetContextWebInformation: { FormDigestValue: string; FormDigestTimeoutSeconds: number };
    }>;
    ...
    if (isDefault) {
      ...
      this.digestExpiry = Date.now() + contextInfo.FormDigestTimeoutSeconds * 1000 - 60000;
    }
    return token;
  }
  ...
  async getSiteUsers(signal?: AbortSignal): Promise<UserInfo[]> {
    return await this.request<UserInfo[]>(`/_api/web/siteusers?$filter=PrincipalType eq 1`, {
      signal,
    });
  }
  ...
  async searchUsers(query: string, signal?: AbortSignal): Promise<UserInfo[]> {
    ...
    try {
      const response = await this.request<{ ClientPeoplePickerSearchUser: string }>(
        pickerEndpoint,
        {
          method: 'POST',
          body: payload,
      ...
      const pickerResults = JSON.parse(response.ClientPeoplePickerSearchUser) as Array<{
        DisplayText: string;
    ...
    if (finalResults.length < 5 && query.length > 2) {
      try {
        ...
        siteUsers.forEach((u) => {
          ...
          const isMatch = lowerQueryParts.every(
            (part) => titleLower.includes(part) || emailLower.includes(part),
          );
          ...
          if (isMatch) {
            const exists = finalResults.some(
              (existing) =>
                ...
                (existing.LoginName &&
                  u.LoginName &&
                  existing.LoginName.toLowerCase() === u.LoginName.toLowerCase()),
            );
  ...
  private async getUserByEmail(email: string): Promise<UserInfo> {
    const users = await this.request<UserInfo[]>(`/_api/web/siteusers?$filter=Email eq '${email}'`);
    const user = users[0];
    if (!user) throw new Error(`User with email ${email} not found on this site.`);
    return user;
  }
  ...
  getVersionHistoryLinkByItem(listId: string, itemId: number, webUrl?: string): string {

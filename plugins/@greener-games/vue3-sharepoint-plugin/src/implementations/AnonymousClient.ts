...
export class AnonymousClient implements ISharePointClient {
  ...
  private async getRequestDigest(customBaseUrl?: string): Promise<string> {
    ...
    try {
      ...
      if (!res.ok) return '';
      const d = (await res.json()) as SPVerboseResponse<{
  ...
  async search<T>(opts: SearchRequestOptions, signal?: AbortSignal): Promise<SearchResult<T>> {
    ...
    if (!scope) {
      this.logger.error(
        "SimpleRest Search requires a specific 'scope' pointing to a list or folder.",
      );
    ...
    // Select
    const selectFields = opts.selectFields || [
      'Title',
      'Id',
      'EncodedAbsUrl',
      'FileRef',
    ...
    const mappedSelects = selectFields
      .map((f) => {
    ...
    try {
      const results = await this.request<Array<Record<string, unknown>>>(endpoint, {
  ...
  private async notSupported(operation: string): Promise<never> {
    ...
    return Promise.reject(
      new Error(`Operation '${operation}' is not supported in SimpleRest Mode.`),
    );
  }

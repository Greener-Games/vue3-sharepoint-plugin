...
export class PnPSharePointClient implements ISharePointClient {
    ...
    try {
      ...
      const searchResultsRaw = await this.sp.search({
        ...
        SortList: options.sortList?.map((s) => ({
          Property: s.property,
          Direction:
            s.direction === 'ascending' ? SortDirection.Ascending : SortDirection.Descending,
        })),
        ...
        await Promise.all(
          items.map(async (item) => {
            if (
              ...
              const itemId =
                typeof item.ListItemId === 'string'
                  ? parseInt(item.ListItemId, 10)
                  : item.ListItemId;
  ...
  async updateFileMetadata(
    ...
    const validator = item as unknown as {
      validateUpdateListItem: (
        vals: unknown,
      ) => Promise<Array<{ ErrorCode: number; FieldName: string; ErrorMessage: string }>>;
    };
  ...
  async getWebInfo(_abortSignal?: AbortSignal): Promise<WebInfo> {
    const w = (await this.sp.web.select(
      'Id',
      'Title',
      'Url',
      'Description',
    )()) as unknown as WebInfo;
    return {
      Id: w.Id,
      Title: w.Title,
      Url: w.Url,
  ...
  async getSubwebs(_abortSignal?: AbortSignal): Promise<WebInfo[]> {
    const webs = (await this.sp.web.webs.select(
      'Id',
      'Title',
      'Url',
      'Description',
  ...
  async getLists(_abortSignal?: AbortSignal): Promise<ListInfo[]> {
    const lists = (await this.sp.web.lists.select(
      'Id',
      'Title',
      'Description',
      'ItemCount',
  ...
  async getList(listTitle: string, _abortSignal?: AbortSignal): Promise<ListInfo> {
    const l = (await this.sp.web.lists
      .getByTitle(listTitle)
      .select(
        'Id',
        'Title',
  ...
  async addUserToGroup(
    ...
    await this.sp.web.siteGroups.getByName(groupName).users.add(user.LoginName);
  }
  ...
  async searchTerm(
    ...
    try {
      ...
      const data = (await res.json()) as {
        value: Array<{ id: string; name: string; names?: Array<{ name: string }> }>;
      };
  ...
  private buildKql(opts: SearchRequestOptions): string {
    ...
    if (opts.fileTypes?.include?.length) {
      parts.push(`(${opts.fileTypes.include.map((e) => `FileExtension:${e}`).join(' OR ')})`);
    }
    if (opts.fileTypes?.exclude?.length) {
      parts.push(`(${opts.fileTypes.exclude.map((e) => `NOT FileExtension:${e}`).join(' AND ')})`);
    }

...
export function useSearch<T = Record<string, unknown>>(
  ...
  const execute = async (opts: SearchRequestOptions) => {
    ...
    const safeOptions = JSON.parse(JSON.stringify(opts)) as SearchRequestOptions;
  ...
  const pageInfo = computed(() => {
    if (!results.value || !_lastOptions.value) return { current: 1, total: 0, pageSize: 10 };
  ...
  const hasNext = computed(() => {
    ...
    return results.value.startRow + (_lastOptions.value.rowLimit || 10) < results.value.totalHits;
  });
  ...
  const nextPage = () => {
    ...
    const nextRow = (results.value?.startRow || 0) + (_lastOptions.value.rowLimit || 10);

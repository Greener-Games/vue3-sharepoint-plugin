<template>
  <div class="dashboard-container">
    ...
    <!-- PAGINATION -->
    <div v-if="totalHits > 20" class="pagination-controls">
...
const formattedResults = computed(() => {
  ...
  return results.value.items.map((item: Record<string, unknown>) => {
    ...
    return {
      title: (item.Title as string) || (item.Filename as string) || 'Untitled',
      path: item.Path as string,
      type: (item.DocumentType as string) || (item.FileExtension as string) || 'Doc',

    ...
    <div class="search-hero">
      <div class="search-content">
        ...
        <div class="search-input-group">
          ...
          <button class="search-btn" :disabled="isSearching" @click="triggerSearch">
    ...
    <div class="main-layout">
      ...
      <div class="results-column">
        ...
        <div class="results-wrapper">
          ...
          <div class="results-list" :class="{ 'is-loading': isSearching }">
            <div v-if="resultsList.length === 0 && !isSearching" class="no-results">
              No documents found.
            </div>
            ...
            <div v-for="doc in resultsList" :key="doc.id" class="result-item">
              ...
              <div class="doc-content">
                ...
                <div class="meta-grid">
                  <div><span class="meta-label">Region:</span> {{ doc.region }}</div>
                  <div><span class="meta-label">Division:</span> {{ doc.division }}</div>
                  <div><span class="meta-label">Practice:</span> {{ doc.practice }}</div>
        ...
        <div v-if="totalHits > 0" class="pagination-footer">
          <button :disabled="!hasPrevPage" @click="changePage(currentPage - 1)">Previous</button>
      ...
      <aside class="refiners-sidebar">
        <div class="sidebar-header">
          ...
          <button v-if="hasActiveFilters" class="clear-btn" @click="clearAllFilters">
            Clear all Filters
          </button>
        </div>

        <!-- 1. STANDARD FILTERS LOOP -->
        <div v-for="filter in standardFilters" :key="filter.key" class="filter-card">
          <div class="filter-card-header" @click="toggleSection(filter.key)">
            <span>{{ filter.title }}</span>
            <span class="chevron" :class="{ open: sections[filter.key] }">›</span>
          </div>
          ...
          >
            <button class="summary-bin-btn" @click.stop="clearSpecificFilter(filter.key)">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
            ...
            <div class="summary-text-wrapper">
              <span class="summary-text">{{ formatSelectionText(filter.key) }}</span>
            </div>
        ...
        <transition name="expand">
          <div v-if="showAdvanced" class="advanced-section">
            <div v-for="filter in advancedFilters" :key="filter.key" class="filter-card">
              <div class="filter-card-header" @click="toggleSection(filter.key)">
                <span>{{ filter.title }}</span>
                <span class="chevron" :class="{ open: sections[filter.key] }">›</span>
              </div>
...
const resultsList = computed(
  () => (searchCtx.results.value?.items as unknown as SearchDoc[]) || [],
);
...
const DIVISIONS = ['Infrastructure', 'Transportation', 'Energy', 'Water', 'Defence'];
const PRACTICES = ['Project Management', 'Engineering', 'Consulting', 'Digital', 'Architecture'];
const MARKETS = ['UK&I', 'North America', 'Middle East', 'Asia Pacific'];
const CAPABILITIES = ['Rail Systems', 'Net Zero', 'Digital Twin', 'Cyber Security'];
...
const standardFilters = computed(() => filterConfigs.filter((f) => !f.isAdvanced));
const advancedFilters = computed(() => filterConfigs.filter((f) => f.isAdvanced));
...
const formatSummary = (content: string) => {
  ...
  return content.replace(/<c0>/g, '<mark class="highlight">').replace(/<\/c0>/g, '</mark>');
};
